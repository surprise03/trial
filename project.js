// Initial trend array
// let trends = JSON.parse(localStorage.getItem("trends")) || [];

// Sidebar Buttons
const homeBtn = document.getElementById("home");
const trendingBtn = document.getElementById("trending");
const postBtn = document.getElementById("post");

// Containers
const homeContainer = document.getElementById("home_container");
const trendingContainer = document.getElementById("Trending_container");
const postContainer = document.getElementById("post_container");
const postedTrends = document.getElementById("postedTrends");


//URL
const url = "http://localhost:3000/content"


// Show Sections
function setActive(button) {
    document.querySelectorAll(".side_container p").forEach(p => p.classList.remove("active"));
    button.classList.add("active");
}

function showSection(section) {
    homeContainer.style.display = "none";
    trendingContainer.style.display = "none";
    postContainer.style.display = "none";

    if (section == trendingContainer) {
        section.style.display = "flex"
    }
    else {
        section.style.display = "block"
    }

}

homeBtn.addEventListener("click", () => {
    showSection(homeContainer);
    setActive(homeBtn);
});

trendingBtn.addEventListener("click", () => {

    showSection(trendingContainer);
    setActive(trendingBtn);
    renderTrending(); // Show posts on trending
});

postBtn.addEventListener("click", () => {
    showSection(postContainer);
    setActive(postBtn);
});

async function fetchUrl() {
    try {
        const response = await fetch(url)
        if (response.ok) {
            const data = await response.json()
            console.log(data)
            const partsOfData = displayData(data)
        }

    } catch (err) {
        console.error(err)
    }




}



// function displaydata(data){
//     trendingContainer.innerHTML=``;
//     const div=document.createElement("div")
//     console.log(data)
//     div.id="dataDisplay"
//     div.innerHTML=`
//          <p>${data.Topic}</p>
//          <p>${data.Title}</p>
//          <p>${data.discription} </p>
//     `;
//     trendingContainer.appendChild(div)
//  }

function displayData(dataArray) {
    trendingContainer.innerHTML = ``; // clear once

    dataArray.forEach(data => {
        const div = document.createElement("div");
        const mainDiv = document.createElement("div")
        mainDiv.className = "mainDiv"
        div.className = "dataDisplay"; // use class if multiple items
        div.innerHTML = `
            
           
            <p>${data.Title}</p>
            <p>${data.description}</p> <!-- check spelling -->
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat" viewBox="0 0 16 16" onclick='comments("${data.id}")'  style="cursor: pointer;" >
            <path d="M2.678 11.894a1 1 0 0 1 .287.801 11 11 0 0 1-.398 2c1.395-.323 2.247-.697 2.634-.893a1 1 0 0 1 .71-.074A8 8 0 0 0 8 14c3.996 0 7-2.807 7-6s-3.004-6-7-6-7 2.808-7 6c0 1.468.617 2.83 1.678 3.894m-.493 3.905a22 22 0 0 1-.713.129c-.2.032-.352-.176-.273-.362a10 10 0 0 0 .244-.637l.003-.01c.248-.72.45-1.548.524-2.319C.743 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7-3.582 7-8 7a9 9 0 0 1-2.347-.306c-.52.263-1.639.742-3.468 1.105"/>
            </svg>
            <p>${data.comments.length}</p>
            
        `;
        trendingContainer.appendChild(div);
    });
}

fetchUrl()

const form = document.getElementById("trendForm")

async function submitdata() {
    const Title = document.getElementById("trendTitle").value
    const description = document.getElementById("trendDesc").value
    const comments=[]

    const response = await fetch(url, {
        "method": "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify({ Title, description,comments})
    })

}

async function comments(id) {
    try {
        const urlFetch = await fetch(`${url}/${id}`);
        const data = await urlFetch.json();

        
        // Access and log the comments
        const commentsList = data.comments;
        // console.log("Comments:", commentsList);
    
        // Optional: Loop through comments if needed
        commentsList.forEach((comment, index) => {
          console.log(`Comment ${index + 1}: ${comment}`);
        });
    
      } catch (error) {
        console.error("Error fetching comments:", error);
      }
    
}





// Show home initially
showSection(homeContainer);
